...
// The parent class that can be inherited into a class to provide
// database functionality to the class.
//The idea is that I want to be able to replace the text included in the methods with sql scripts that can be run elsewhere
class DBTable {
...
public:
    ...
    // methods for storing the base sql commands, these methods
...
int cb_load(void  *data,
            int    argc,
            char **argv,
            char **azColName);

int cb_store(void  *data,
             int    argc,
             char **argv,
             char **azColName);
